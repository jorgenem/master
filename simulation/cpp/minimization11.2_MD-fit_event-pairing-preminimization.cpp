...
void evaluate_simplex(double ** simplex, int dim,double * fx,  double (*func)(double *, int, vector<int>, int, double, bool, vector<bool> &, vector<vector<mat>> &, vector<vector<vec>> &, vector<int>),
	int Nevents, vector<int> iEvents, int jBin, double Mnorm, bool combinatorics, vector<bool> &all_leptons_equal_list, vector<vector<mat>> &D_lists, vector<vector<vec>> &E_lists, vector<int> combinatorics_choice)
{
	int i;
	for (i = 0; i < dim + 1; i++)
	{
		// correct_combinatorics.clear();
		fx[i] = (*func)(simplex[i], Nevents, iEvents, jBin, Mnorm, combinatorics, all_leptons_equal_list, D_lists, E_lists, combinatorics_choice);
	}
}
...
int update_simplex(double * point, int dim, double & fmax,double * midpoint, double * line, double scale, double (*func)(double *, int, vector<int>, int, double, bool, vector<bool> &, vector<vector<mat>> &, vector<vector<vec>> &, vector<int>),
	int Nevents, vector<int> iEvents, int jBin, double Mnorm, bool combinatorics, vector<bool> &all_leptons_equal_list, vector<vector<mat>> &D_lists, vector<vector<vec>> &E_lists, vector<int> combinatorics_choice)
{
	int i, update =	0; 
	...
	// correct_combinatorics.clear();
	fx = (*func)(next, Nevents, iEvents, jBin, Mnorm, combinatorics, all_leptons_equal_list, D_lists, E_lists, combinatorics_choice);
	if (fx < fmax)
	{
		for (i = 0; i < dim; i++)	
			point[i] = next[i];
...
void contract_simplex(double ** simplex, int dim, double * fx, int ilo, double (*func)(double *, int, vector<int>, int, double, bool, vector<bool> &, vector<vector<mat>> &, vector<vector<vec>> &, vector<int>), int Nevents, vector<int> iEvents, int jBin, double Mnorm, bool combinatorics, vector<bool> &all_leptons_equal_list, vector<vector<mat>> &D_lists, vector<vector<vec>> &E_lists, vector<int> combinatorics_choice)
{
	int i, j;
	for (i = 0; i < dim + 1; i++)
		if (i != ilo)
...
bool amoeba(double *point, double &fmin, double (*func)(double *, int, vector<int>, int, double, bool, vector<bool> &, vector<vector<mat>> &, vector<vector<vec>> &, vector<int>), 
	...
	int Nevents, vector<int> iEvents, int jBin, double Mnorm, bool combinatorics, vector<bool> &all_leptons_equal_list, vector<vector<mat>> &D_lists, vector<vector<vec>> &E_lists, vector<int> combinatorics_choice)
{
	...
	evaluate_simplex(simplex, dim, fx, func, 
		Nevents, iEvents, jBin, Mnorm, combinatorics, all_leptons_equal_list, D_lists, E_lists, combinatorics_choice);
	...
	while (iter < maxiter)
	{
		...
		if (fx[ihi] < fx[ilo])
			...
			if (!update_simplex(simplex[ihi], dim, fx[ihi], midpoint, line, 0.5, func, Nevents, iEvents, jBin, Mnorm, combinatorics, all_leptons_equal_list, D_lists, E_lists, combinatorics_choice))
				contract_simplex(simplex, dim, fx, ilo, func, Nevents, iEvents, jBin, Mnorm, combinatorics, all_leptons_equal_list, D_lists, E_lists, combinatorics_choice);
		iter += 1;
	}
...
double xisquared(double *Masses, int Nevents, vector<int> iEvents, int j, double Mnorm, bool combinatorics, vector<bool> &all_leptons_equal_list, vector<vector<mat>> &D_lists, vector<vector<vec>> &E_lists, vector<int> combinatorics_choice)
{
	vec M;
	...
	// Loop over the current subselection of sample events (2, initially)
	for (int i = 0; i < iEvents.size(); i++)
	{
		int iEvent = iEvents[i] + j*Nevents;

		double xisquared_current;
		vec P;
		P = D_lists[combinatorics_choice[i]][iEvent]*M + E_lists[combinatorics_choice[i]][iEvent];
		xisquared_current = pow(P[3]*P[3] - P[0]*P[0] - P[1]*P[1] - P[2]*P[2] - MLSPsq, 2) + pow(P[7]*P[7] - P[4]*P[4] - P[5]*P[5] - P[6]*P[6] - MLSPsq, 2);
		P = D_lists[combinatorics_choice[i]+1][iEvent]*M + E_lists[combinatorics_choice[i]+1][iEvent];
		...
		P = D_lists[combinatorics_choice[i]+2][iEvent]*M + E_lists[combinatorics_choice[i]+2][iEvent];
		xisquared_current = xisquared_current + pow(P[3]*P[3] - P[0]*P[0] - P[1]*P[1] - P[2]*P[2] - MLSPsq, 2) + pow(P[7]*P[7] - P[4]*P[4] - P[5]*P[5] - P[6]*P[6] - MLSPsq, 2);
		P = D_lists[combinatorics_choice[i]+3][iEvent]*M + E_lists[combinatorics_choice[i]+3][iEvent];
		xisquared_current = xisquared_current + pow(P[3]*P[3] - P[0]*P[0] - P[1]*P[1] - P[2]*P[2] - MLSPsq, 2) + pow(P[7]*P[7] - P[4]*P[4] - P[5]*P[5] - P[6]*P[6] - MLSPsq, 2);
...
void best_fit(int Nbins, int Nevents, string eventfile, vector<double> masses_initial, double tol, int maxiter, bool combinatorics, double Mnorm, vector<vector<vector<vector<double> > > > &best_fit_value, vector<vector<vector<vector<vector<double> > > > > &best_fit_point, vector<double> &correct_combinatorics_fraction, vector<vector<vector<vector<int> > > > &pair_indices)
{
	int N = Nbins*Nevents;
	// cout << "N = " << endl;
	...
	// Loop to minimize every combination of m events among Nbins total events for every combination of 
	// A matrices (2 or 4 depending on OFL/SFL)

	// best_fit_point should contain an entry for each sample, which again contains an entry for every event, which again contains an entry for every combination with m other events, which again contains an entry for every combination of A's. Pjuh.

	int m = 2; // number of events to combine for pre-minimization (initially 2)
	int possible_combinations_of_m_events = 4; // 4 if you are minimizing pairs of events

	for (int iBin = 0; iBin < Nbins; iBin++)
	{

		vector<vector<vector<vector<double> > > > best_fit_point_currentbin;
		vector<vector<vector<double> > > best_fit_value_currentbin;
		vector<vector<vector<int> > > pair_indices_currentbin;

		for (int iEvent = 0; iEvent < Nevents; iEvent++)
		{

			// Allocate vector for storing all pairings of iEvent with others
			vector<vector<vector<double> > > best_fit_point_currentevent;
			vector<vector<double> > best_fit_value_currentevent;
			vector<vector<int> > pair_indices_currentevent; // Vector to store tuples of event indices to keep track of each pairing
	
			for (int iPairing1 = 0; iPairing1 < Nevents-1; iPairing1++) // Index of first event to "pair" with. Add additional loops if you want to minimize three or 	more events at a time.
			{

				int iPairing1Index;
				if (iPairing1 < iEvent)
					iPairing1Index = iPairing1;
				else if (iPairing1 >= iEvent)
					iPairing1Index = iPairing1+1;
				else
				{
					cout << "Error!";
					exit;
				}
				cout << "Pairing event " << iEvent+1 << " with event " << iPairing1Index+1 << endl;

				vector<int> iEvents = {iEvent, iPairing1Index}; // These are the indices (inside the sample) of the events to include in current minimization
				pair_indices_currentevent.push_back(iEvents);
	
				// If more than two events are combined: Extra loops go above here!
				// Allocate vector to store all combinations of A matrices for current paring of events
				vector<vector<double> > best_fit_point_currentpair;
				vector<double> best_fit_value_currentpair;
	
				for (int iCombination1 = 0; iCombination1 < 2; iCombination1++)
				{
	
					for (int iCombination2 = 0; iCombination2 < 2; iCombination2++)
					{
	
						vector<int> combinatorics_choice = {iCombination1, iCombination2};

						vector<double> masses_current = masses_initial;
	
						double fmin;
						if (amoeba(&masses_current[0], fmin, xisquared, tol, maxiter, m, iEvents, iBin, Mnorm, combinatorics, all_leptons_equal_list, D_lists, 	E_lists, combinatorics_choice)) 
						{
							...
							{
								best_fit_value_currentpair.push_back(1e30);
								best_fit_point_currentpair.push_back({0,0,0,0});
								// bin_number_currentpair.push_back(iBin);
								continue;
							}		
							best_fit_point_currentpair.push_back(Masses);
							best_fit_value_currentpair.push_back(fmin);
							// pair_indices_currentpair.push_back({iBin});
				...
				} // END loop over chain combinations in current pair

				best_fit_point_currentevent.push_back(best_fit_point_currentpair);
				best_fit_value_currentevent.push_back(best_fit_value_currentpair);
	
			} // END loop over pairings with current event

		best_fit_point_currentbin.push_back(best_fit_point_currentevent);
		best_fit_value_currentbin.push_back(best_fit_value_currentevent);
		pair_indices_currentbin.push_back(pair_indices_currentevent);

		} // END loop over events

		best_fit_point.push_back(best_fit_point_currentbin);
		best_fit_value.push_back(best_fit_value_currentbin);
		pair_indices.push_back(pair_indices_currentbin);
	...
	//// OLD VERSION:
	// for (int combinatorics_choice = 0; combinatorics_choice < 2; combinatorics_choice++)
	// {
	...
	// 		double fmin;
	// 		if (amoeba(&masses_current[0], fmin, xisquared, tol, maxiter, Nevents, iEvents, iBin, Mnorm, combinatorics, 	all_leptons_equal_list, D_lists, E_lists, combinatorics_choice)) 
	// 		{
	// 			// Calculate masses from MLSP + squared-diffs
	// 			double mllinv = 80; // Calculated from true masses using formula
	// 			double MLSPsq = masses_current[2]*(masses_current[1]/(mllinv*mllinv) - 1.0);
...
int main()
{
	...
	vector<vector<vector<vector<double> > > > best_fit_value;
	vector<vector<vector<vector<vector<double> > > > > best_fit_point; 
	vector<double> correct_combinatorics_fraction;
	vector<vector<vector<vector<int> > > > pair_indices;
	...
	best_fit(Nbins, Nevents, eventfile, massdiff, tol, maxiter, combinatorics, Mnorm, best_fit_value, best_fit_point, correct_combinatorics_fraction, pair_indices);
	...
	ofstream textOutput;
	textOutput.open("../best_fit_results/TEMP-preevent_minimization.dat", ios::out);
	textOutput << "# Minimization of all pairs of events within each sample/bin, using mass-difference minimization with dilep edge and summing 4 closest combinations." << endl;
	textOutput << "# Event file name = " << eventfile << ", SIMPLEX tolerance = " << tol << endl;

	int iBin = 0;
	for (int iEvent = 0; iEvent < Nevents; iEvent++)
	{	
		for (int iPair = 0; iPair < Nevents-1; iPair++)
		{
			textOutput << iEvent+1 << "\t" << iPair+1 << "\t" << best_fit_value[iBin][iEvent][iPair][0] << "\t" << best_fit_value[iBin][iEvent][iPair][1] << "\t" << best_fit_value[iBin][iEvent][iPair][2] << "\t" << best_fit_value[iBin][iEvent][iPair][3] << endl;
		}
	}	
