...
int main()
{
	...
	vector<vector<double> > masses_initial_vector = {{568, 180, 144, 97},{400, 300, 200, 100},{800, 500, 300, 50},{1000, 100, 80, 30}};
	double Mnorm = 100;
	double tol = 1e-12;
	double maxiter = 2000;

	vector<const char*> tempname_vector = {"../best_fit_results/TEMP_TMP.dat", "../best_fit_results/TEMP_400-300-200-100.dat", "../best_fit_results/TEMP_800-500-300-50.dat", "../best_fit_results/TEMP_1000-100-80-30.dat"};

	for (int iSP = 0; iSP < 4; iSP++)
	{
		vector<double> masses_initial = masses_initial_vector[iSP];
		const char* tempname = tempname_vector[iSP];
		...
		ofstream textOutput;
		textOutput.open(tempname, ios::out);
