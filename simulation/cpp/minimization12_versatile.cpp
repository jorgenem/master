...
void best_fit(int Nbins, int Nevents, string eventfile, vector<double> masses_initial, double tol, int maxiter, bool combinatorics, double Mnorm, vector<double> &best_fit_value, vector<vector<double> > &best_fit_point, vector<double> &correct_combinatorics_fraction, vector<int> &bin_number, int minimization_method, int combosum, double detAcut, vector<double> &detA_passcut_fraction)
{
	...
	for (int iBin=0; iBin<Nbins; iBin++)
	{
		...
		// Begin loop(s) over subsets of sample, e.g. to iterate towards best combinatorics
		...
		// {
			...
			// TEST: Fill eventIndices with all events in bin and only correct combo
			int iCombo = 0;
			for (int iEvent = iBin*Nevents; iEvent < (iBin+1)*Nevents; iEvent++)
			{
				if (!detApasscut_list[iEvent][iCombo]) // Skip events which don't pass detA cut
					continue;
				eventIndices.push_back(make_pair(iEvent,iCombo));
			}
...
int main()
{
	...
	best_fit(Nbins, Nevents, eventfile, masses_initial, tol, maxiter, combinatorics, Mnorm, best_fit_value, best_fit_point, correct_combinatorics_fraction, bin_number, minimization_method, combosum, detAcut, detA_passcut_fraction);
