...
file = open("../best_fit_results/TEMP-subdetAcut.dat",'r')
...
mchi1_passcut = []
detAcut_passcut = []
cut = 50000 # xi^2 cut value in units of (100 GeV)^4
...
print "subdetA passcut-fraction:", (np.mean(detAcut_passcut)*100)
...
plt.text(236, 554, r"$\mathrm{subdet}(A)_\mathrm{min} = 1$", fontsize=13)
plt.text(236, 540, r"$f_{\mathrm{subdet}(A)} = %d \%%$" % (np.mean(detAcut_passcut)*100), fontsize=14)
